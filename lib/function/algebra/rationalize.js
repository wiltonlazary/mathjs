...
/** 
 * auxiliary log function to print expression in readable form
 * in javascript syntax
 *
 * Syntax
 *    striComm(varName,s,size)
 *
 * @param  {string} varName    variable name
 * @param  {object|string} s   node or string expression
 * @param  {number} size       size of output for breaking in pieces
 *
 *
 * @return string              expression divided is small pieces
 */
function striComm(varName, s, size) {
var saida="";
var piece;
  ...
  function polyToCanonical(node,coefficients) {
    ...
    function recurPol(node,noPai,o) {
      ...
      if (tp==='FunctionNode')            // ***** FunctionName *****
        ...
        //     throw new ArgumentsError('Invalid ' + node.op +  ' placing in ' + stri(node))
